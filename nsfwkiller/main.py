...
from aiogram.client.bot import DefaultBotProperties   # ← yeh naya import
...
# Updated Bot initialization (aiogram 3.7+ / 3.26+ style)
bot = Bot(
    token=BOT_TOKEN,
...
async def start_cmd(message: types.Message):
    help_text = (
        ...
        "✅ Real-time 18+ detection on photos, videos, GIFs, stickers, documents\n"
        "?? Auto delete + optional mute/ban\n"
        "?? Live stats with Redis\n"
        "?? Colored premium buttons + custom emoji\n\n"
        "<b>Commands:</b>\n"
        "/start  → Yeh help message\n"
        "/stats  → Total deleted + CPU/RAM\n\n"
        "<i>Bot ko group mein Admin banao (Delete Messages + Restrict Members)</i>"
    )
...
async def main():
    await init_db()
    logger.info("?? NSFW Killer Bot Started Successfully (aiogram 3.26+ fixed)")
